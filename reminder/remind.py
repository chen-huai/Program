...
#------------------def-------------------  
def get_remind():  
    f2 = open(r'./reminder.txt')
    lines = f2.readlines()    
    for line in lines:
        day_list.append(line.split('-')[0])
        time_lists.append(line.split('-')[1])
        reminder_lists.append(line.split('-')[2])

def showMessage(reminder):  
    ...
    #窗口显示的文字、并设置字体、字号  
    label = Label(frame,text="%s"%reminder,font="微软雅黑 -20 bold")  
...
while True:  
    ...
    last_time=(datetime.datetime.now()-datetime.timedelta(minutes=10)).strftime("%H:%M")
    next_time=(datetime.datetime.now()+datetime.timedelta(minutes=10)).strftime("%H:%M")
    n=0
    for list in time_lists:
        if day_list[n]=='%s'%today or day_list[n]=='workday' or day_list[n]=='everyday':        
            if last_time<='%s'%time_lists[n]<=next_time:
                showMessage('%s'%reminder_lists[n])
        n+=1
    time.sleep(10*60-2)
